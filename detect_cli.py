...
def main():
    parser = argparse.ArgumentParser(
        description="CLI for detecting and reviewing anomalies in CI/CD logs"
    )
    parser.add_argument(
        "input",
    ...
    parser.add_argument(
        "-k", "--openai-key",
        help="OpenAI API key (if not specified, taken from env OPENAI_API_KEY)"
    )
    parser.add_argument(
        "-t", "--threshold",
        type=float,
        default=0.5,
        help="anomaly probability threshold (0–1, default=0.5)"
    )
    parser.add_argument(
        "-d", "--describe",
        action="store_true",
        help="generate anomaly overview via OpenAI"
    )
    parser.add_argument(
        "-m", "--model",
        default="model_supervised.pkl",
        help="path to model file (default=model_supervised.pkl)"
    )
    ...
    print(f"Anomalies found: {len(anomalies)} (threshold={args.threshold})\n")
    ...
    if args.describe:
        print("\nGenerating anomaly overview via OpenAI…")
        try:
